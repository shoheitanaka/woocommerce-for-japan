...
class WC_Gateway_PPEC_IPS_Handler {
	...
	public function maybe_received_credentials() {
		...
		if ( ! wp_verify_nonce( $_GET['wc_ppec_ips_admin_nonce'], 'wc_ppec_ips' ) ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized,WordPress.Security.ValidatedSanitizedInput.MissingUnslash
			wp_die( __( 'Invalid connection request', 'woocommerce-for-japan' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		if ( ! empty( $_GET['error'] ) ) {
			...
			$this->_redirect_with_messages( __( 'Sorry, Easy Setup encountered an error.  Please try again.', 'woocommerce-for-japan' ) );
		}
		...
		try {
			...
			if ( ! $payer_id ) {
				$this->_redirect_with_messages( __( 'Easy Setup was able to obtain your API credentials, but was unable to verify that they work correctly.  Please make sure your PayPal account is set up properly and try Easy Setup again.', 'woocommerce-for-japan' ) );
			}
		} catch ( PayPal_API_Exception $ex ) {
			$error_msgs[] = array(
				'warning' => __( 'Easy Setup was able to obtain your API credentials, but an error occurred while trying to verify that they work correctly.  Please try Easy Setup again.', 'woocommerce-for-japan' ),
			);
		}

		$error_msgs[] = array(
			'success' => __( 'Success!  Your PayPal account has been set up successfully.', 'woocommerce-for-japan' ),
		);
