...
class WC_Gateway_PPEC_Client_Credential_Certificate extends WC_Gateway_PPEC_Client_Credential {
	...
	public function configure_curl( $handle, $r, $url ) {
		...
		if ( false === curl_setopt( $handle, CURLOPT_SSLCERT, $certificate_file ) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
			throw new Exception( __( 'Unable to accept certificate during cURL configuration', 'woocommerce-for-japan' ), WC_Gateway_PPEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
		...
		if ( $this->_use_secure_transport() && false === curl_setopt( $handle, CURLOPT_SSLCERTPASSWD, $password ) ) { // phpcs:ignore WordPress.WP.AlternativeFunctions.curl_curl_setopt
			throw new Exception( __( 'Unable to accept certificate password during cURL configuration', 'woocommerce-for-japan' ), WC_Gateway_PPEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	}
	...
	protected function _maybe_create_certificate_file( $password ) {
		...
		if ( ! $temp_file ) {
			...
			throw new Exception( sprintf( __( 'Unable to write certificate file %s during cURL configuration', 'woocommerce-for-japan' ), $temp_file ), WC_Gateway_PPEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	...
	protected function _maybe_create_secure_certificate_file( $temp_file, $password ) {
		...
		if ( false === $private_key ) {
			throw new Exception( __( 'Failed to retrieve private key during cURL configuration', 'woocommerce-for-japan' ), WC_Gateway_PPEC_Client::INVALID_ENVIRONMENT_ERROR );
		}

		if ( ! openssl_pkcs12_export_to_file( $this->_certificate, $temp_file, $private_key, $password ) ) {
			throw new Exception( __( 'Failed to export PKCS12 file during cURL configuration', 'woocommerce-for-japan' ), WC_Gateway_PPEC_Client::INVALID_ENVIRONMENT_ERROR );
		}
	}
