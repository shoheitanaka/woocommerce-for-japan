...
class PayPal_API_Exception extends Exception {
	...
	public function __construct( $response ) {
		parent::__construct( __( 'An error occurred while calling the PayPal API.', 'woocommerce-for-japan' ) );
		...
		foreach ( $errors as $value ) {
			...
			$error_messages[] = sprintf( __( 'PayPal error (%1$s): %2$s', 'woocommerce-for-japan' ), $error->error_code, $error->maptoBuyerFriendlyError() );
		}

		if ( empty( $error_messages ) ) {
			$error_messages[] = __( 'An error occurred while calling the PayPal API.', 'woocommerce-for-japan' );
		}
